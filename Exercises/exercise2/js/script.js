...
function mousePressed() {
  //if state is title, clicking changes it to simulation
  if (state === `title`) {
    state = `simulation`;
  }

  //if state is simulation, then annyang says drug names
  if (state === `simulation`) {
    //choose a random drug from list
    currentDrug = random(DRUGS);

    //reverse current drug
    let reverseDrug = reverseString(currentDrug);

    //use responsive Voice API to say reverseDrug
    responsiveVoice.speak(reverseDrug);
  }

  if (state === `simulation` && score > MAXSCORE) {
    state = `end`;
  }
}
